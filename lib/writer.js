...
class Writer {
  ...
  createSingleFile () {
    ...
    // get all css content
    // FixMe: quick fix replace with updated bootstrap version. Temporarily replacing 3x package with a local reference to the CSS
    // const bootstrapCss = this.fs.readFileSync(this.findAsset('bootstrap/dist/css/bootstrap.min.css'), 'utf8');
    const bootstrapCss = this.fs.readFileSync(this.path.join(this.opt.template, 'src', 'css', 'bootstrap.min.css'), 'utf8');
