...
/**
 * PostProcess API Group
 *
 * Processes the parsed files and metadata after the initial processing is completed.
 * Sets group names, titles, and descriptions based on defined groups.
 *
 ...
 * @param {object} preProcess - Pre-processing results containing defined groups.
  ...
  parsedFiles.forEach(function (parsedFile, parsedFileIndex) {
    parsedFile.forEach(function (block) {
      ...
      if (Object.keys(block.global).length === 0) {
        ...
        // set title before cleaning group name
        block.local[`${target}Title`] = group;
...
*/
        ...
        matchedData.title = block.local[target + 'Title'];
      ...
      if (matchedData.description) {
        block.local.groupDescription = matchedData.description;
      } // keep original block.local
    });
  });
}
